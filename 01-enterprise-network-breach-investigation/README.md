...
The following values are confirmed by the completed lab and the supplied screenshots:

| Finding | Verified Value |
|---|---|
| External IP with the most reconnaissance activity | `203.0.113.45` |
| Internal host targeted by scans | `10.0.0.20` (`FINANCE-SRV1`) |
| VPN account targeted | `svc_backup` |
| Internal IP assigned after successful VPN login | `10.8.0.23` |
| Port used for lateral SMB attempts | `445` |
| Internal host beaconing to C2 | `10.0.0.60` (`WORKSTATION-60`) |
| External C2 IP | `198.51.100.77` |
| Host showing exfiltration attempts | `10.0.0.51` (`APP-WEB-01`) |

The current evidence screenshots are documented in [`screenshots/README.md`](screenshots/README.md).
...
Counting blocked requests showed `203.0.113.45` producing **279 blocked events**, substantially more than the other sources shown in the supplied material. The completed lab also identified `10.0.0.20` (`FINANCE-SRV1`) as the internal host targeted by the scans.
...
The evidence supports a VPN credential attack from `203.0.113.45` targeting the `svc_backup` account. The repeated failures were followed by successful authentication, after which `10.8.0.23` was assigned as the internal VPN address.
...
The evidence supports **lateral movement through SMB-related activity over TCP/445**. The supplied screenshots verify the port used for the lateral SMB attempts. They do not establish one single definitive destination host for the successful movement, so this project does not assign one.
...
The recurring outbound pattern and IDS classification support **command-and-control communication** from `10.0.0.60` (`WORKSTATION-60`) to `198.51.100.77:4444`.
...
Firewall activity showed repeated outbound connections from `10.0.0.51` (`APP-WEB-01`) to `198.51.100.77` over ports:
...
1. `203.0.113.45` performed the largest volume of perimeter reconnaissance in the supplied logs and `10.0.0.20` was identified as the internal scan target.
2. `203.0.113.45` generated repeated VPN authentication failures against `svc_backup` followed by successful VPN access and assignment of `10.8.0.23`.
...
6. `10.0.0.60` generated recurring C2 beaconing alerts to `198.51.100.77:4444`.
7. `10.0.0.51` later communicated repeatedly with `198.51.100.77` over ports 80/8080, and related IDS activity was classified as potential data exfiltration.
8. The exact contents and successful receipt of any transferred data are not established by the available evidence, so the project does not claim confirmed data theft.
...
└── screenshots/
    ├── README.md
    ├── 05-c2-beaconing-raw-events.png
    ├── 06-c2-alert-summary.png
    ├── 07-exfiltration-firewall-connections.png
    └── 08-lab-answer-verification.png
```
...
This project documents an **authorized cybersecurity training scenario**. The IP addresses and username shown here are verified from the completed lab and supplied screenshots. The project still separates confirmed observations from claims the evidence does not establish, particularly regarding the contents or successful receipt of potentially exfiltrated data.
